...
function simpan_karyawan()
{
	...
	// select karyawan
	$sk = mysqli_query($koneksi, "SELECT * FROM tb_karyawan WHERE nip='$nip'");
	$ck = mysqli_fetch_row($sk);

	if ($ck) {
		echo "<script>alert('data sudah ada')</script>";
		return header("location: ?m=");
	}else{
		if ($foto!= "") {
		$allowed_ext = array('png','jpg');
		$x = explode(".", $foto);
...
function ubah_jam_keluar()
{
	...
	$update = mysqli_query($koneksi, "UPDATE jam_keluar SET jam = '$jam' WHERE id='$id'");
	if ($update) {
