...
function simpan_absen()
{
	...
	// Validasi tanggal
	$select = mysqli_query($koneksi, "SELECT * FROM tb_absen WHERE nip='$nip' AND tanggal='$tanggal' AND absen_masuk='1'");
...
function val_absen_k(){
	global $koneksi;
	date_default_timezone_set("Asia/Jakarta");
  	$tanggalSekarang = date("d-m-Y");
	$id  = $_SESSION['idabsensikaryawan'];
	$query1 = mysqli_query($koneksi, "SELECT * FROM tb_karyawan WHERE id = '$id'");
	$row1 = mysqli_fetch_array($query1);

	// nip
	$nip = $row1['nip'];

	$query2 = mysqli_query($koneksi, "SELECT * FROM tb_absen WHERE nip = '$nip' AND tanggal = '$tanggalSekarang'");
...
function simpan_absen_keluar(){
	...
	$query1 = mysqli_query($koneksi, "SELECT * FROM tb_absen WHERE nip='$nip' AND tanggal='$tanggal' AND absen_keluar = 1");
	$check = mysqli_num_rows($query1);

	if ($check) {
		echo "<script>alert('maaf, anda sudah absen keluar hari ini')</script>";
	}else{
		$query2 = mysqli_query($koneksi, "UPDATE tb_absen SET jam_keluar='$jam', absen_keluar='1' WHERE nip ='$nip'");
		if ($query2) {
			echo "<script>alert('Terima kasih, anda telah absen keluar')</script>";
		}else{
...
function select_absen(){
	...
	return $query2 = mysqli_query($koneksi, "SELECT * FROM tb_absen WHERE nip = '$nip' AND tanggal = '$tanggalSekarang'");
}
